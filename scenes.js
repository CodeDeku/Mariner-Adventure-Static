// --- SCENE DATABASE (Extracted from Index-Stable) ---
export const scenes = {
    // --- PROLOGUE ---
    'prologue_storm': {
    ...
    'prologue_abyss': {
        ...
        text: "A wave of pure magical force shatters the hull. The freezing dark claims you instantly. You sink, drifting into the abyss, and see IT rising—a colossal shadow, vast as a mountain. The screams of Gareth, Quinn, Talon, and Bones are silenced by the water... only to re-ignite inside your skull, burning hotter than the freezing deep.",
        journal: "I died. I know I died. But the Shadow beneath the waves spat me back out. Now, I am not alone in my own skin.",
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 5);
    ...
    'start': {
        ...
        journal: "Woke up choking on sand. Gareth's hat is in my hand—I can't let go of it. The Compass is screaming, and the crew... they are loud. So loud. They don't know they're ghosts.",
        banter: {
            'quinn': "The manifest! The logs! We can't be erased, Edward. Find the records or we never existed!"
        },
        onEnter: (state) => {
            window.audioManager.playGasp();
            let msg = "";
    ...
    'wreckage_success': {
        title: "Salvage",
        text: "Quinn guides your hand to a buried chest. Inside, a pouch of <span class='text-item'>Shillings</span> and a <span class='text-item'>Rusted Cutlass</span>. You strap the blade to your belt—the rust flakes away as your hand touches the hilt, remembering a fight you haven't fought yet. Quinn points frantically to a damp book lying nearby—the <span class='text-item'>Ship's Log</span>.",
    ...
    'trail_fail': {
        ...
        onEnter: (state) => { 
            state.currentHP = window.applyDamage(state.currentHP, 2); 
            window.fxManager.flashRed();
    ...
    'guard_intimidate_check': {
        ...
        resolve: (roll, state) => {
            ...
            if (state.inventory.includes("Captain's Tricorn")) {
                finalRoll += 1;
                // Since this logic runs in scenes.js, we assume addToHistory is global or handled by the wrapper.
                // However, renderScene handles the history logging for rolls usually.
            }
    ...
    'combat_guard': {
        ...
        onEnter: (state) => { 
            ...
            state.currentHP = window.applyDamage(state.currentHP, dmg);
            window.fxManager.flashRed(); 
            state.influence = 50;
    ...
    'smuggler_cargo': {
        ...
        onEnter: (state) => {
            ...
            // Stock access via window or assumed stable structure
            // NOTE: scenes object is exported, so 'scenes' variable inside here might not refer to the global object.
            // Using window.scenes is safer.
            if(window.scenes && window.scenes['market_stall']) window.scenes['market_stall'].stock["Curious Cargo"] = 0;
    ...
    'buy_rum': {
        ...
        onEnter: (state) => { 
            state.inventory.push("Rum"); 
            window.scenes['market_stall'].stock["Rum"] -= 1;
            window.audioManager.playCoins();
            return "Bought Rum."; 
        },
    ...
    'buy_potion': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['market_stall'].stock["Healing Potion"] -= 1;
            window.audioManager.playCoins();
    ...
    'buy_rope': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['market_stall'].stock["Rope"] -= 1;
            window.audioManager.playCoins();
            return "Bought Rope."; 
        },
        choices: [{ text: "Return to Merchant", nextScene: 'market_stall', type: 'travel', doomCost: 0 }]
    ...
    'buy_torch': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['market_stall'].stock["Torch"] -= 1;
            window.audioManager.playCoins();
            return "Bought Torch."; 
        },
        choices: [{ text: "Return to Merchant", nextScene: 'market_stall', type: 'travel', doomCost: 0 }]
    ...
    'sell_cargo': {
        ...
        onEnter: (state) => {
            const idx = state.inventory.indexOf("Curious Cargo");
            if (idx > -1) state.inventory.splice(idx, 1);
            window.scenes['market_stall'].stock["Curious Cargo"] = 0;
            state.shillings += 10; 
            window.audioManager.playCoins();
    ...
    'sell_rope': {
        ...
        onEnter: (state) => {
            ...
            if (idx > -1) state.inventory.splice(idx, 1);
            state.shillings += 2;
            window.scenes['market_stall'].stock["Rope"] = (window.scenes['market_stall'].stock["Rope"] || 0) + 1;
            window.audioManager.playCoins();
    ...
    'sell_torch': {
        ...
        onEnter: (state) => {
            ...
            if (idx > -1) state.inventory.splice(idx, 1);
            state.shillings += 1;
            window.scenes['market_stall'].stock["Torch"] = (window.scenes['market_stall'].stock["Torch"] || 0) + 1;
            window.audioManager.playCoins();
    ...
    'sell_potion': {
        ...
        onEnter: (state) => {
            ...
            if (idx > -1) state.inventory.splice(idx, 1);
            state.shillings += 3;
            window.scenes['market_stall'].stock["Healing Potion"] = (window.scenes['market_stall'].stock["Healing Potion"] || 0) + 1;
            window.audioManager.playCoins();
    ...
    'sell_rum': {
        ...
        onEnter: (state) => {
            ...
            if (idx > -1) state.inventory.splice(idx, 1);
            state.shillings += 2; 
            window.scenes['market_stall'].stock["Rum"] = (window.scenes['market_stall'].stock["Rum"] || 0) + 1;
            window.audioManager.playCoins();
    ...
    'dark_fail': {
        ...
        onEnter: (state) => { 
            state.currentHP = window.applyDamage(state.currentHP, 4); 
            window.fxManager.flashRed();
    ...
    'lighthouse_vigil': {
        ...
        isRest: true,
        onEnter: (state) => {
            // Using global addToHistory if possible, or assuming renderScene handles it
    ...
    'captain_orders': {
        ...
        choices: [
            ...
            { 
                ...
                customClass: 'btn-game spectral-active',
                onSelect: (state) => { 
                    state.currentHP = 25; // Hardcoded max overheal
    ...
    'overlook_scan_success': {
        title: "Talon's Eye",
        text: "Talon guides your focus. You spot a discarded pack near a large rock formation—a traveler was robbed or fled quickly. It holds a small but valuable Silver Locket.",
    ...
    'town_descent': {
        ...
        journal: "The Compass cracked. It's pulling me apart. Smelter fire to the East, Chapel fog to the West. Gareth is right—I can't save everyone. I have to choose.",
        banter: {
            'gareth': "Hold fast! A Captain who chases two headings sails in circles! Choose a target, Mister Jones. Fire or Fog—pick your poison!"
        },
        choices: [
    ...
    'town_square': {
        ...
        onEnter: (state) => {
            ...
            if (state.act3Started) {
                ...
                window.scenes['town_square'].text = desc;
                ...
                window.scenes['town_square'].choices = act3Choices;
            ...
            if (hasRing && hasBlade) {
                window.scenes['town_square'].text = "The <span class='text-void'>Rift</span> is tearing the sky apart. Violet lightning strikes the cobblestones. You have the <span class='text-item'>Ring</span> and the <span class='text-item'>Blade</span>. The Ritual is the only way to close this wound.";
                ...
                window.scenes['town_square'].choices = newChoices;
                return "Finale Unlocked.";
            }
            
            else if (hasRing) {
                window.scenes['town_square'].text = "You return with the <span class='text-item'>Ring of Brass</span>. Fog from the Sunken Chapel is flooding the streets. The <span class='text-void'>Shadow threat</span> has escalated. You must pursue the Fire Path now.";
            } 
            else if (hasBlade) {
                window.scenes['town_square'].text = "You return with the <span class='text-item'>Sword of Valor</span>. Ash falls from the Smelter District like <span class='text-fire'>gray snow</span>. The <span class='text-fire'>Fire threat</span> has escalated. You must pursue the Shadow Path now.";
            } 
            else {
                window.scenes['town_square'].text = "The Port is in chaos. Civilians flee toward the docks. You check the <span class='text-item'>Compass</span>. The needle splits violently, vibrating between two distinct sources of planar interference:<br><br>1. An intense <span class='text-fire'>Heat Signature</span> from the Smelter District.<br>2. A deep <span class='text-void'>Void Pocket</span> in the Sunken Chapel.<br><br>To stabilize the Rift, you must investigate both.";
            }
            
            window.scenes['town_square'].choices = newChoices.filter(choice => {
    ...
    'alchemist_triage': {
        ...
        text: "Madam Alara, encased in her beaked mask, tends to the wounded with the cold efficiency of a butcher. She doesn't offer comfort, only chemistry. Her wares are potent—she deals only in true survival.",
    ...
    'alchemist_secret_stock': {
        ...
        text: "Madam Alara takes the Strange Coin—a brass piece from an ancient kingdom—and her head snaps up. 'I recognize this metal. It's from the kingdom of the Wolf King,' she hisses. 'You honor my trade. This Draft is my reserve, a single draught that cheats the ferryman. Use it wisely, Mariner.'",
    ...
    'buy_fire': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['alchemist_triage'].stock["Alchemical Fire"] -= 1;
            window.audioManager.playCoins();
    ...
    'buy_water': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['alchemist_triage'].stock["Holy Water"] -= 1;
            window.audioManager.playCoins();
    ...
    'buy_tincture': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['alchemist_triage'].stock["Vitality Tincture"] -= 1;
            window.audioManager.playCoins();
    ...
    'buy_salts': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['alchemist_triage'].stock["Smelling Salts"] -= 1;
            window.audioManager.playCoins();
            return "Bought Salts."; 
        },
    },
    'buy_powder': {
        ...
        onEnter: (state) => { 
            ...
            window.scenes['alchemist_triage'].stock["Flash Powder"] -= 1;
            window.audioManager.playCoins();
    ...
    'sell_potion_alch': {
        ...
        onEnter: (state) => {
            ...
            if (idx > -1) state.inventory.splice(idx, 1);
            state.shillings += 4;
            window.scenes['alchemist_triage'].stock["Healing Potion"] = (window.scenes['alchemist_triage'].stock["Healing Potion"] || 0) + 1;
            window.audioManager.playCoins();
    ...
    'sell_tincture': {
        ...
        onEnter: (state) => {
            ...
            if (idx > -1) state.inventory.splice(idx, 1);
            state.shillings += 7; 
            window.scenes['alchemist_triage'].stock["Vitality Tincture"] = (window.scenes['alchemist_triage'].stock["Vitality Tincture"] || 0) + 1;
            window.audioManager.playCoins();
    ...
    'blackwater_tavern': {
        ...
        journal: "The Blackwater stank of defeat. But Quinn was right—the walls have ears. I sat in the dark and listened to the dying city breathe.",
        banter: {
    ...
    'eat_gruel_instant': {
        ...
        onEnter: (state) => {
            ...
            if (window.scenes['blackwater_tavern'].stock["Gruel"] > 0) {
                window.scenes['blackwater_tavern'].stock["Gruel"] -= 1;
            }
    ...
    'tavern_gambler': {
        ...
        text: "With the Ring's power, you hear it—or rather, you don't. The man in the corner has no heartbeat. The cards slide into his sleeve with supernatural grace. He is a corpse playing at life. You confront him.",
    ...
    'town_docks': {
        ...
        onEnter: (state) => {
            const workBtn = window.scenes['town_docks'].choices.find(c => c.nextScene === 'docks_work');
            if (workBtn) {
    ...
    'docks_dive_success': {
        ...
        text: "You hold your breath until your lungs burn, leveraging the dock pilings to dive deep. You surface with a watertight strongbox containing a pouch of gold and gems—enough to buy any artifact the Alchemist sells.",
    ...
    'smelter_damage': { 
        ...
        onEnter: (state) => { 
            state.currentHP = window.applyDamage(state.currentHP, 5, 'fire'); 
            window.fxManager.flashRed();
    ...
    'smelter_containment_success': {
        ...
        text: "You slam the heavy blast doors shut, sealing the fire—and the workers—inside. Their screams are cut short by the roar of the furnaces. It is a cold calculus, but the primary objective is now more secure. You spot a bucket of Industrial Lubricant they dropped outside the door.",
    ...
    'smelter_save_workers_fail': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 4, 'psychic');
            window.fxManager.flashRed();
            return "Took 4 Psychic Damage from the crew's screams.";
        },
    },
    
    // --- MULTI-STAGE BOSS FIGHT: SLAG HORROR ---
    'smelter_stage1_start': {
        title: "Slag Horror: Stage 1 - The Heat Wave",
        text: "The <span class='text-danger'>Slag Horror</span> rises from the molten iron, its core glowing. It prepares to spew a <span class='text-fire'>wave of magma</span>. You must find a way to mitigate the heat to get close enough to strike. You spot the hilt of the <span class='text-item'>Sword of Valor</span> suspended in its chest.",
        ...
        banter: {
            'gareth': "It's winding up! Don't stand there—move!"
        },
        isBoss: true,
        choices: [
            { 
                text: "Throw Holy Water", 
                nextScene: 'smelter_victory_item',
                type: 'action', 
                reqItem: 'Holy Water',
                consume: true,
                customClass: 'btn-success-glow'
            ...
            { 
                text: "Throw Flash Powder and Flee", 
                nextScene: 'town_square', 
                type: 'action', 
                reqItem: 'Flash Powder', 
                consume: true, 
                doomCost: 1,
                action: (state) => {
                    state.isBossCombat = false;
                    return "Escaped the Slag Horror.";
                }
            }
        ]
    },
    'smelter_dodge_logic': {
        ...
        resolve: (roll, state) => {
            if (roll >= 14) {
                 state.pendingDamage = 4;
                 return 'smelter_stage2_damaged';
            } else {
                 state.pendingDamage = 8;
                 return 'smelter_stage2_damaged';
            }
        }
    },
    ...
    'smelter_stage2_damaged': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, state.pendingDamage || 0, 'fire');
            window.fxManager.flashRed();
    ...
    'smelter_victory_lubricant': {
        ...
        onEnter: (state) => {
            ...
            let logMsg = "Acquired Sword of Valor.";

            if (state.inventory.includes("Rusted Cutlass")) {
                const cutlassIdx = state.inventory.indexOf("Rusted Cutlass");
                if (cutlassIdx > -1) state.inventory.splice(cutlassIdx, 1);
                
                // Using global helper if needed, or assume renderScene handles
            } 
    ...
    'smelter_victory_brute': {
        ...
        onEnter: (state) => {
            state.isAbsorbCharged = false;
            state.currentHP = window.applyDamage(state.currentHP, 10);
            window.fxManager.flashRed();
    ...
    'chapel_side_torch': {
        ...
        text: "The flame crackles, momentarily thinning the black fog. You see a set of stairs leading to an upper level—a quick path! On a dry shelf near the top, you find a Pearlescent Mushroom, left to dry.",
    ...
    'chapel_damage': { 
        ...
        ,
        onEnter: (s) => { 
            s.currentHP = window.applyDamage(s.currentHP, 6); 
            window.fxManager.flashRed();
    ...
    'chapel_vestry': {
        ...
        text: "The vestry is flooded with black, polluted water. A panicked man is caught in the undertow, struggling to keep his head above the foul tide. He screams for help. The water is necrotic—Edward's mind screams for him not to touch it.",
    ...
    'chapel_rescue_success': {
        ...
        text: "You pull the panicked man from the black water. Gasping, he hands you a small, heavy silver heart—a family locket. 'Sell this, please. For my family.'",
    ...
    'chapel_ignore_drowning': {
        ...
        text: "You ignore the man's screams. He drowns quickly. You search his floating corpse and find only a damp, useless map, but Talon spots something shining behind the altar—a consecrated bottle of Holy Water, untouched by the black tide.",
    ...
    'chapel_stage1_reveal': {
        ...
        text: "The crypt is flooded. The <span class='text-danger'>Drowned Spectre</span> is present but invisible in the thick, churning <span class='text-void'>black fog</span>. It will strike from the dark unless you can pinpoint its true location. On its skeletal finger glints the <span class='text-item'>Ring of Brass</span>.",
        ...
        banter: {
            'talon': "It's circling us. Stop looking with your eyes. Watch for the displacement in the fog."
        },
        isBoss: true,
        choices: [
            { 
            ...
            { 
                ...
                nextScene: 'chapel_stage2_finish',
                type: 'action', 
                reqItem: 'Ring of Brass',
                customClass: 'btn-keen-senses' 
            },
            ...
            { 
                ...
                action: (state) => {
                    state.isBossCombat = false;
                    return "Escaped the Drowned Spectre.";
                }
            }
        ]
    },
    ...
    'chapel_stage2_hit_damaged': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, state.pendingDamage || 0);
            window.fxManager.flashRed();
    ...
    'chapel_victory_brute': {
        ...
        onEnter: (state) => {
            state.isSurpriseRound = false;
            state.currentHP = window.applyDamage(state.currentHP, 12, 'psychic');
            window.fxManager.flashRed();
            if (!state.inventory.includes("Ring of Brass")) state.inventory.push("Ring of Brass");
            return "Took 12 Psychic DMG. Acquired Ring of Brass.";
        },
    },
    ...
    'rift_collapse_punishment': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 6);
            window.fxManager.flashRed();
    ...
    'act2_arrival': {
        ...
        onEnter: (state) => {
            ...
            const lanternDef = window.itemRegistry["Spectral Lantern"];
    ...
    'act2_tether_fail': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 8, 'psychic');
    ...
    'cub_tame_check': {
        ...
        resolve: (roll, state) => {
            ...
            if(state.inventory.includes('Gruel')) {
                dc = Math.max(5, dc - 3);
                // Assume addToHistory available or wrap it
    ...
    'cub_tame_fail': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 3);
            window.fxManager.flashRed();
    ...
    'beast_blink_distract': {
        ...
        text: "You whistle sharply. Blink phases out of your pack, appearing behind the massive Displacer Beast. He chirps—a sound that cuts through the ether. The beast spins around, confused by the familiar planar signature. <br><br>While the mother is distracted by the cub, you slip past into the shadows of the Hall. Blink rejoins you moments later, vibrating with adrenaline.",
    ...
    'beast_fail': {
        ...
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 8);
            window.fxManager.flashRed();
    ...
    'staircase_fail': {
        ...
        text: "You climb for hours—or maybe seconds. You vomit from the vertigo and find yourself back at the bottom. The Tether weakens.",
        onEnter: (state) => {
    ...
    'manor_forge': {
        ...
        text: "The Manor Kitchen hearth burns with a ghostly violet flame, powered by residual planar energy. This cold fire fuses matter with memory—the perfect place to customize your gear. You examine the cold stone table.",
        choices: [
    ...
    'manor_forge_combine': {
        ...
        choices: [
        {
                ...
                customClass: 'btn-fire-glow',
                onSelect: (state) => {
                    // Using combineItems from window context since this is executed in global scope context via renderChoices usually
                    // Or we assume the function is available globally.
                    // For safety, assume global combineItems if available, otherwise just use standard logic
                    // NOTE: combineItems is defined in index.html. Scenes.js cannot see it directly unless it's on window.
                    // We must update index.html to put combineItems on window.
                    return window.combineItems("Flash Powder", "Alchemical Fire", "Sun-Star Grenade");
                }
            },
            {
                ...
                customClass: 'btn-success-glow',
                onSelect: (state) => {
                    return window.combineItems("Rum", "Vitality Tincture", "Captain's Reserve");
                }
            },
            {
                ...
                customClass: 'btn-keen-senses',
                onSelect: (state) => {
                    return window.combineItems("Rope", "Beast Whistle", "Snare Trap");
                }
            },
            {
                ...
                customClass: 'btn-success-glow',
                onSelect: (state) => {
                    return window.combineItems("Volatile Mutagen", "Vitality Tincture", "Stabilized Mutagen");
                }
            },
            {
                ...
                customClass: 'btn-success-glow',
                onSelect: (state) => {
                    return window.combineItems("Rum", "Pearlescent Mushroom", "Mind-Expanding Grog");
                }
            },
            {
                ...
                customClass: 'btn-fire-glow',
                onSelect: (state) => {
                    return window.combineItems("Sword of Valor", "Mimic's Bile", "Acid-Etched Sword of Valor");
                }
            },
            {
                ...
                customClass: 'btn-hero',
                onSelect: (state) => {
                    return window.combineItems("Sword of Valor", "Holy Water", "Radiant-Washed Sword of Valor");
                }
            },
            {
                ...
                onSelect: (state) => {
                    return window.combineItems("Spectral Lantern", "Alchemical Fire", "The Warding Ember");
                }
            },
            {
                ...
                customClass: 'btn-hero',
                onSelect: (state) => {
                    return window.combineItems("Sword of Valor", "Deep-Iron Ingot", "Deep-Forged Cutlass");
                }
            },
            {
                ...
                customClass: 'btn-keen-senses',
                onSelect: (state) => {
                    return window.combineItems("Rope", "Brass Gears", "Grapple Hook");
                }
            },
        ...
        choices: [
            {
                ...
                state.shipName = name; 
                window.renderScene('act3_intro_ship'); 
    ...
    'sea_map_hub': {
        ...
        onEnter: (state) => {
            ...
            // 1. STARVATION CHECK
            if (state.supplies < 2) {
                window.scenes['sea_map_hub'].text = "The hold is empty. You must drift and fish to survive.";
                window.scenes['sea_map_hub'].choices = [
            ...
            window.scenes['sea_map_hub'].text = "You stand at the wheel. Where to, Captain?";
            ...
            window.scenes['sea_map_hub'].choices = navChoices;
    ...
    'sea_flotsam_event': {
        ...
        text: "You spot debris drifting in the current—remnants of a ship that didn't make it. Barrels bob in the swell among the <span class='text-item'>Flotsam</span>. It's dangerous to haul them in while moving, but there might be supplies.",
        banter: {
    ...
    'travel_to_port': {
        ...
        resolve: (roll, state) => {
            ...
            if (state.inventory.includes("Gilded Sextant")) {
                ...
                // addToHistory global or handled by renderScene logic
            }
            state.supplies -= cost;
            state.currentLocation = 'port_hub';
            ...
            if (rng < 0.1) return 'sea_scavenger_encounter'; // 10% chance
            if (window.gameFunctions.checkForContraband() && rng < 0.5) return 'sea_navy_encounter';
    ...
    'sea_navy_caught': {
        ...
        onEnter: (state) => {
            ...
            state.inventory = state.inventory.filter(name => {
                const item = window.itemRegistry[name];
    ...
    'sea_fog_success': {
        ...
        text: "Through skill or spectral light, you find the seam in the mist. The jagged teeth of the reef loom out of the whiteout—inches from the hull—before fading behind you. You have broken the fog's grip.",
    ...
    'bleak_dogtooth': {
        ...
        text: "This jagged, wind-whipped island is home to the <span class='text-danger'>Lost Pack</span>. Once noble guardians, they are now broken things, patrolling the black rocks and snarling at the sea. Their fur is matted with black ichor.<br><br>The <span class='text-item'>Ring of Brass</span> begins to throb against your finger. It is a slow, rhythmic heat—like a heartbeat calling out to its own kind.",
        onEnter: (state) => {
            state.currentLocation = 'dogtooth';
    ...
    'dogtooth_intimidate_logic': {
        ...
        resolve: (roll, state) => {
                ...
                if (state.inventory.includes("Captain's Tricorn")) {
                roll += 1;
                // No history log here as renderScene handles basic roll logging, but we can assume logic holds
                }
    ...
    'silas_sanctuary': {
        ...
        onEnter: (state) => {
            ...
            if (isEthereal) {
                window.scenes['silas_sanctuary'].text = "Silas stands before you, his armor shining with spectral light. He is waiting, but he is a ghost to you right now. 'You must return, Mariner. You cannot swear an oath to me from the void.'";
                window.scenes['silas_sanctuary'].choices = [
                    { 
                ...
                window.scenes['silas_sanctuary'].text = "The cliff is solid behind you. Silas stands in the real world now, holding out the hilt of his sword. 'Welcome back, Mariner. Now, the oath.'";
                window.scenes['silas_sanctuary'].choices = [
                    { 
    ...
    'dogtooth_lab_ruins': {
        ...
        text: "Hidden in a sea-cave, you find the remains of a laboratory. Cages line the walls—too small for men, too large for dogs. Broken glass crunches underfoot.<br><br>This is where they made the plague. Amidst the ruin, you see a pristine silver <span class='text-item'>Beast Whistle</span> and a syringe glowing with <span class='text-void'>Volatile Mutagen</span>.",
    ...
    'barracks_clue_found': {
        ...
        text: "Quinn finds a discrepancy in the supply logs. A Quartermaster is smuggling maps out of the Citadel. But he isn't taking money—he's taking **Nutrient Slurry**.<br><br>The note reads: *'I have the patrol routes. Bring the dose to the Rib-Cage Tavern. The white veins are spreading to my face.'*",
    ...
    'hub_chaos': {
        ...
        text: "Your boots sink slightly as you step off the gangplank. This isn't rock. The floating island has a texture like cured leather, vast and pockmarked. The shantytown built upon it is carved from materials that look suspiciously like ribcages and teeth.<br><br>The air smells copper-sharp—like blood and ozone. Scavengers are everywhere, hacking into the ground beneath their feet with pickaxes, harvesting glowing, gelatinous chunks of the island itself.",
    ...
    'sea_scavenger_encounter': {
        ...
        onEnter: (state) => {
            if (state.inventory.includes("Feeding Ground Map")) {
                window.scenes['sea_scavenger_encounter'].text = "The diving bell breaches again. The man inside sees you and waves a rusted gauntlet. 'You have the map... why are you still here? Go before *It* wakes up.'";
                window.scenes['sea_scavenger_encounter'].choices = [
    ...
    'order_petrified_ward': {
        ...
        banter: {
            'bones': "The Surgeon is a butcher. Look at that boy in the corner—the coral hasn't hit the lymph nodes yet. If I excise the root *now*, I can save the limb and the life. (Medicine Check)"
        },
        choices: [
            { 
    ...
    'ward_medicine_success': {
        ...
        journal: "Performed a life-saving surgery. Bones was right—it can be cut out.",
        onEnter: (state) => {
            state.shillings += 10;
    ...
    'ward_bloom_event': {
        ...
        text: "You approach a young soldier. He looks at you, trying to speak, but a massive spike of white coral bursts through his jaw. His back arches, ribs snapping as the reef takes him completely. <br><br>The Surgeon screams, 'He's blooming! Hold him down!' The soldier—no longer a man, but a <span class='text-danger'>Coral-Husk</span>—thrashes violently, razor-sharp barnacles erupting from his skin.",
    ...
    'ward_bloom_fail': {
        title: "Too Late",
        text: "The coral growth is explosive. A spike catches you in the shoulder, throwing you back. By the time you recover, the soldier is gone—fully consumed. The guards rush in with flamethrowers to incinerate the corpse.",
        onEnter: (state) => {
            state.currentHP = window.applyDamage(state.currentHP, 5);
            state.wardBloomDone = true;
            window.fxManager.flashRed();
    ...
    'coral_hulk_intro': {
        ...
        text: "You step into the corrupted chamber. A Royal Guard Dreadnought—massive steam armor—but it is fused with white coral. The organic matter pulses, knitting metal back together. It roars, a wet, grinding sound.",
        isBoss: true,
        onEnter: (state) => {
            window.initCombat("Coral-Hulk", 80, [6, 12]); 
            return "Combat Started! The enemy regenerates 5 HP per round.";
        },
        choices: [
            { text: "Engage the Monstrosity", nextScene: 'coral_hulk_combat', type: 'travel', doomCost: 0 }
        ]
    },
    ...
    'coral_hulk_combat': {
        ...
        choices: [
            ...
            { 
                text: "Strike with Sword of Valor", 
                action: () => window.resolveCombatRound('valor_strike'),
                type: 'action', 
                reqItem: 'Sword of Valor',
                customClass: 'btn-hero'
            },
            { 
                text: "Desperate Strike (Cutlass)", 
                action: () => window.resolveCombatRound('heavy_strike'),
                type: 'action', 
                reqNotItem: 'Sword of Valor'
            },

            // C. ALLY COMMANDS (Only available if joined)
            {
                text: "Command Silas (Intercept)",
                type: 'action',
                reqFlag: 'silasJoined',
                action: () => window.allyCommand('silas'),
                doomCost: 0
            },
            {
                text: "Command Blink (Phase Shift)",
                type: 'action',
                reqFlag: 'hasBlinkSkill',
                action: () => window.allyCommand('blink'),
                doomCost: 0
            },
            
            // D. ITEM ACTIONS (Mirror Shield Reflect)
            { 
                text: "Mirror Shield (Reflect Stance)", 
                action: () => window.resolveCombatRound('reflect_stance'),
                type: 'action', 
                reqItem: 'Mirror Shield',
                customClass: 'btn-success-glow'
            },
            { 
                text: "Scale-Stake Driver (Stun)", 
                action: () => window.resolveCombatRound('item_stake_driver'),
                type: 'action', 
                reqItem: 'Scale-Stake Driver',
                customClass: 'btn-fire-glow'
            },
            
            // E. CONSUMABLES (Strategic Nuke Buttons)
            { 
                text: "Throw Alchemical Fire", 
                action: () => window.resolveCombatRound('item_fire'),
                type: 'action', 
                reqItem: 'Alchemical Fire',
                consume: true,
                customClass: 'btn-fire-glow'
                ...
                { 
                text: "Throw Holy Water", 
                action: () => window.resolveCombatRound('item_holy_water'),
                type: 'action', 
                reqItem: 'Holy Water',
                consume: true,
                customClass: 'btn-success-glow'
    ...
    'sanguine_spire_combat': {
        ...
        text: "You enter the cathedral. Three <span class='text-danger'>Vampire Elders</span> descend from the vaulted ceiling. They move with blurring speed, their claws dripping with ancient rot.<br><br>The <span class='text-item'>Sword of Valor</span> hums violently in your hand—it knows this blood. Silas steps forward, shield raised. Blink growls, phasing in and out. This is the end.",
        journal: "Confronted the Vampire Elders. The final battle.",
        isBoss: true,
        onEnter: (state) => {
            window.initHordeCombat("Vampire Elder", 35, [4, 7], 3); 
            return "Combat Started! Three Elders attack per round.";
        },
        choices: [
            { text: "Engage the Horde", nextScene: 'spire_combat_loop', type: 'travel', doomCost: 0 }
        ]
    },
    ...
    'spire_combat_loop': {
        ...
        onEnter: (state) => {
            ...
            if (state.activeEnemies.length === 1) {
                window.scenes['spire_combat_loop'].text = `Only the last Elder remains! The veil is torn open—enter the Ethereal Plane to strike them with <span class='text-spectral'>Force Damage</span>.`;
            } else if (state.activeEnemies.length > 1) {
                window.scenes['spire_combat_loop'].text = `Two Elders remain! They phase in and out, gaining <span class='text-danger'>30% Evasion</span>. Use Blink or Plane Walk to guarantee strikes.`;
            } else {
                window.scenes['spire_combat_loop'].text = `Three Elders remain!`;
            }
            
            // --- 2. BUILD CHOICES (Horde-specific generation) ---
            let dynamicChoices = [];
            
            // A. ATTACKS (Target-Specific buttons for living Elders)
            state.activeEnemies.forEach(elderName => {
                // Valor Strike (If Sword of Valor is present)
                if (state.inventory.includes('Sword of Valor')) {
                    dynamicChoices.push({
                        text: `Valor Strike (${elderName})`,
                        action: () => window.resolveHordeRound('valor_strike', elderName),
                        type: 'action',
                        customClass: 'btn-hero'
                    });
                } else {
                    // Heavy Strike (Fallback)
                    dynamicChoices.push({
                        text: `Heavy Strike (${elderName})`,
                        action: () => window.resolveHordeRound('heavy_strike', elderName),
                        type: 'action'
                    });
                }
            });
            
            // B. ALLY COMMANDS (No target needed)
            ...
            // C. AOE/ITEM ACTIONS (No target needed - Damage applies to all)
			if (state.inventory.includes('Holy Water')) dynamicChoices.push({ text: "Throw Holy Water (AOE)", action: () => window.resolveHordeRound('item_holy_water', 'none'), type: 'action', reqItem: 'Holy Water', consume: true, customClass: 'btn-success-glow' });
			if (state.inventory.includes('Alchemical Fire')) dynamicChoices.push({ text: "Throw Alchemical Fire (AOE)", action: () => window.resolveHordeRound('item_fire', 'none'), type: 'action', reqItem: 'Alchemical Fire', consume: true, customClass: 'btn-fire-glow' });
			if (state.inventory.includes('Sun-Star Grenade')) dynamicChoices.push({ text: "Throw Sun-Star (AOE Stun)", action: () => window.resolveHordeRound('item_sun_star', 'none'), type: 'action', reqItem: 'Sun-Star Grenade', consume: true, customClass: 'btn-fire-glow' });
			
			// D. STUN/CC (No target needed)
					if (state.inventory.includes('Flash Powder')) dynamicChoices.push({ text: "Throw Flash Powder (AOE Stun/Skip)", action: () => window.resolveHordeRound('item_flash', 'none'), type: 'action', reqItem: 'Flash Powder', consume: true });
					if (state.inventory.includes('Scale-Stake Driver')) dynamicChoices.push({ text: "Fire Stake Driver (Stun)", action: () => window.resolveHordeRound('item_stake_driver', 'none'), type: 'action', reqItem: 'Scale-Stake Driver', customClass: 'btn-fire-glow' });


					// E. HEALING (Resolves enemy attack)
					if (state.inventory.includes('Vitality Tincture')) {
						dynamicChoices.push({ 
							...
							action: (state) => {
								...
								window.addToHistory("Drank Tincture. Healed 15 HP.", 'combat');
								window.resolveHordeRound('none', 'none'); // Trigger enemy turn
							} 
						});
					}
					
					// F. PLANAR TOGGLE (No combat action, no enemy retaliation)
					dynamicChoices.push({
					...
					window.scenes['spire_combat_loop'].choices = dynamicChoices;
            ...
            'spire_victory': {
                ...
                text: "The last Elder shrieks as it crumbles to dust. The **Rift Core Fragment**—a splinter of violet light, the final piece of your soul—floats free. You grasp it. The voices of your crew surge, demanding you return the fragment to the Manor's Anchor and finish the mission.",
            ...
            'wolf_estate_safe': {
                ...
                onEnter: (state) => {
                    ...
                    if (state.inventory.includes("Rift Core Fragment") && state.isEthereal) {
                        window.scenes['wolf_estate_safe'].text = "You stand in the Grand Foyer, holding the **Rift Core Fragment**. The entire Vanishing Manor is vibrating violently, urging you toward the Anchor at the top of the Penrose Staircase. The final act begins.";
                        
                        window.scenes['wolf_estate_safe'].choices = [
                            { 
                    ...
                    window.scenes['wolf_estate_safe'].text = "The Manor is sealed in the spectral echo. Here, the planar barriers are reinforced. You are safe. The old halls of the estate stretch out behind you, silent and frozen in memory.";
                    window.scenes['wolf_estate_safe'].choices = baseChoices;
			...
			'manor_foyer_safe': {
                title: "The Grand Foyer",
                text: "The Grand Foyer is silent. The floating staircase hangs motionless in the void, and dust motes dance in the light of your Spectral Lantern. The threats of the past are gone, leaving only the architecture.",
                onEnter: (state) => {
                    state.tetherTimer = 0;
                ...
                choices: [
                    ...
                    { text: "Enter Grand Hall (North)", nextScene: 'manor_hall_safe', type: 'travel', doomCost: 0 },
            ...
            'manor_library_safe': {
				...
				banter: {
					'quinn': "If you bring me data—Lore Fragments, old maps—I can compile a Codex for you. But I need time."
				},
				choices: [
					{ 
			...
			'manor_cellar_success': {
				...
				text: "The mechanism clicks with a satisfying, heavy thud. Pneumatic tubes hiss, and a heavy compartment slides open. Inside, pristine <span class='text-item'>Brass Gears</span> rest on velvet—components stripped from the Wolf King's own clockwork soldiers.",
            ...
            'manor_hall_safe': {
				title: "The Grand Hall",
				text: "The wreckage of the Brass Sentinel lies in a heap. The Penrose Staircase spins slowly in the distance, but the door at the top is sealed shut. The Rift is closed.",
				choices: [
					{ 
            ...
            'act3_long_rest': {
				...
				text: "You collapse into a spectral hammock. The Manor resets itself around you. Talon returns from the walls, and Quinn organizes his fresh notes.",
				isRest: true,
				onEnter: (state) => {
					state.currentHP = state.maxHP;
					Object.keys(state.crew).forEach(k => state.crew[k].used = false);
			...
			'silas_reckoning_combat': {
				...
				onEnter: (state) => {
					state.currentHP = window.applyDamage(state.currentHP, 3); // Guaranteed, low damage
					window.fxManager.flashRed();
					state.silasVengeanceActive = true; 
            ...
            'citadel_reaction_logic': {
                ...
                text: "The Commandant, tears in his eyes, thanks you for ending the Sanguine Tithe. 'They tried to contact us,' he says, 'but the Court is silent. The debt is cancelled! But, Mariner—the white growth on the walls... it's receding. It's too fast. What have you done?'",
                ...
                choices: [
                    { 
                        ...
                        action: (state) => {
                            ...
                            state.influence = -50;
                            window.renderScene('ending_protector');
                        }
                    },
                    { 
                        ...
                        action: (state) => {
                            ...
                            state.influence = 50;
                            window.renderScene('ending_champion');
                        }
                    }
                ]
            },
