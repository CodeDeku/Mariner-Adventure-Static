            ...
            'lighthouse_vigil': {
				...
				onEnter: (state) => {
					const shanty = `The shanty holds the truth. I must find the Ring and the Blade to save my crew. I am the Captain now.<br><br>
					  <i class='spectral-text-glow'>“There’s a tax for the coin and a tax for the bread,<br>
            ...
            'smelter_victory_lubricant': {
                ...
                onEnter: (state) => {
                    ...
                    if (state.inventory.includes("Rusted Cutlass")) {
                        ...
                        addToHistory("Your Rusted Cutlass shattered against the Slag Horror's hide—useless iron against a god of fire. You dropped the burning hilt and grasped the Sword of Valor. Finally, a weapon worthy of the mission.", 'entry');
                        logMsg = "Cutlass destroyed. Acquired Sword of Valor.";
                    } else {
                         addToHistory("You grasped the Sword of Valor from the beast's chest. The steel hummed with the Wolf King's memory.", 'entry');
                    }
			...
			'spire_combat_loop': {
				...
				onEnter: (state) => {
					...
					if (state.activeEnemies.length === 1) {
						scenes['spire_combat_loop'].text = `Only the last Elder remains! The veil is torn open—enter the Ethereal Plane to strike them with <span class='text-spectral'>Force Damage</span>.`;
					...
					if (state.inventory.includes('Flash Powder')) dynamicChoices.push({ text: "Throw Flash Powder (Stun/Skip)", action: () => window.resolveHordeRound('item_flash', 'none'), type: 'action', reqItem: 'Flash Powder', consume: true });
					...
					if (state.inventory.includes('Vitality Tincture')) {
						dynamicChoices.push({ 
							...
							action: (state) => {
								...
								addToHistory("Drank Tincture. Healed 15 HP.", 'combat');
